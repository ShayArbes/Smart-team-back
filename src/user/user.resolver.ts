...
import {Logger, NotFoundException, UseInterceptors, UsePipes, ValidationPipe } from '@nestjs/common';
import { PubSub } from 'graphql-subscriptions';
import { CacheInterceptor, CacheKey } from '@nestjs/cache-manager';
import {GraphqlCacheInterceptor} from './interceptors/graphqlCache.interceptor';
import { GraphqlCacheKey } from 'src/decorator/decorator.decorator';
...
export class UserResolver {
  ...
  async findOneByEmail(@Args('email', { type: () => String }) email: string) {
    try {
      ...
      logger.error(`Error fetching user by email: ${error.message}`, error.stack);
      throw error;
    }
  }
